...
Start the long run by hand when the precursor matrix changes:
...
Quantification uses a preset 0.7, the FAQ's rule for this case, and reports the fitted slope as a
sensitivity run. The preset was chosen before any downstream count was consulted. The notebook
prints both slopes side by side.

The FAQ says a slope set too low makes the analysis more conservative; here the lower fitted slope
gave the smaller standard errors and the larger hit counts.
